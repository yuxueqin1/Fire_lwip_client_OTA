 ...
 * All rights reserved.
 *
 * $Id: mbmrtu.c,v 1.51 2010/11/01 17:19:24 embedded-so.embedded-solutions.1 Exp $
 */
...
{
    UBYTE           ubIdx;
    volatile UBYTE  ubRTUFrameBuffer[MBM_SER_PDU_SIZE_MAX]; //todo:栈的大小是否需要扩大？
...
STATIC BOOL     bMBMSerialRTUTransmitterEmptyAPIV2CB( xMBHandle xHdl, UBYTE * pubBufferOut, USHORT usBufferMax,
...
void            vMBMLogRTUFrame( eMBPortLogLevel eLevel, xMBMInternalHandle * pxIntHdl, char *szMsg,
...
STATIC          eMBErrorCode
eMBMSerialRTUFrameSend( xMBHandle xHdl, UCHAR ucSlaveAddress, USHORT usMBPDULength )    //mark:eMBMSerialRTUFrameSend = pFrameSendFN
{
...
#endif
    {
        ...
        if( MB_IS_VALID_HDL( pxRTUHdl, xMBMRTUFrameHdl ) &&
            MB_IS_VALID_WRITE_ADDR( ucSlaveAddress ) &&
            ( usMBPDULength <= ( MBM_SER_PDU_SIZE_MAX - ( 1 /* Slave Address */  + 2 /* CRC16 */  ) ) ) )
        {
            ...
            if( MB_ENOERR !=
                ( eStatus =
                  eMBPSerialTxEnable( pxRTUHdl->xSerHdl,
                                      ( pbMBPSerialTransmitterEmptyCB ) bMBMSerialRTUTransmitterEmptyAPIV2CB ) ) )
            {
            ...
            if( MB_ENOERR !=
                ( eStatus =
                  eMBPSerialTxEnable( pxRTUHdl->xSerHdl,
                                      ( pbMBPSerialTransmitterEmptyCB ) bMBMSerialRTUTransmitterEmptyAPIV1CB ) ) )
            {
                HDL_RESET_TX( pxRTUHdl );
            }
#else
...
STATIC          eMBErrorCode
eMBMSerialRTUFrameReceive( xMBHandle xHdl, UCHAR ucSlaveAddress, USHORT * pusMBPDULength ) //mark:eMBMSerialRTUFrameReceive = pFrameRecvFN
{
...
#if MBM_SERIAL_API_VERSION == 1
STATIC void
bMBMSerialRTUReceiverAPIV1CB( xMBHandle xHdl, UBYTE ubValue ) //mark:bMBMSerialRTUReceiverAPIV1CB
{
...
STATIC          BOOL
bMBMSerialRTUTransmitterEmptyAPIV1CB( xMBHandle xHdl, UBYTE * pubValue ) //mark:bMBMSerialRTUTransmitterEmptyAPIV1CB
{
    eMBErrorCode    eStatus;
    ...
    case MBM_STATE_TX_XMIT:
        ...
        {
            if( MB_SER_BROADCAST_ADDR == pxRTUFrameHdl->ubRTUFrameBuffer[MBM_SER_PDU_ADDR_OFF] ) // 广播的情况
            {
            ...
            {
                bEnableRx = TRUE;   // 不是广播的情况，发送完后要使能接收
            }
        }
        break;
    ...
    {
#if MBM_RTU_WAITAFTERSEND_ENABLED == 1  
        if( MB_ENOERR != eMBPTimerStart( pxRTUFrameHdl->xTmrWaitHdl ) ) //发送完等一会再接收，为了避免主机收到自己发出去的数据
        {
            eStatus = eMBPEventPost( pxIntHdl->xFrameEventHdl, ( xMBPEventType ) MBM_EV_SEND_ERROR );
            MBP_ASSERT( MB_ENOERR == eStatus );
        }
#else
        if( MB_ENOERR !=
            eMBPSerialRxEnable( pxRTUFrameHdl->xSerHdl, ( pvMBPSerialReceiverCB ) bMBMSerialRTUReceiverAPIV1CB ) ) 
        {
            eStatus = eMBPEventPost( pxIntHdl->xFrameEventHdl, ( xMBPEventType ) MBM_EV_SEND_ERROR );
            MBP_ASSERT( MB_ENOERR == eStatus );
        }
...
#else
        if( MB_ENOERR !=
            eMBPSerialRxEnable( pxRTUFrameHdl->xSerHdl, ( pvMBPSerialReceiverCB ) vMBMSerialRTUReceiverAPIV2CB ) )
        {
            eStatus = eMBPEventPost( pxIntHdl->xFrameEventHdl, ( xMBPEventType ) MBM_EV_SEND_ERROR );
            MBP_ASSERT( MB_ENOERR == eStatus );
        }
...
#endif
    BOOL
bMBMSerialRTUT35CB( xMBHandle xHdl ) //mark:bMBMSerialRTUT35CB
{
    eMBErrorCode    eStatus;
...
#if MBM_RTU_WAITAFTERSEND_ENABLED == 1
STATIC          BOOL
bMBMSerialWaitCB( xMBHandle xHdl ) //mark:bMBMSerialWaitCB
{
    eMBErrorCode    eStatus;
    xMBMInternalHandle *pxIntHdl = xHdl;
    xMBMRTUFrameHandle *pxRTUFrameHdl;
