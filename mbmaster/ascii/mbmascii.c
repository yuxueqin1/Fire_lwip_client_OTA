 ...
 * All rights reserved.
 *
 * $Id: mbmascii.c,v 1.34 2010/11/01 17:13:29 embedded-so.embedded-solutions.1 Exp $
 */
...
STATIC BOOL     bMBMSerialASCIITransmitterEmptyAPIV2CB( xMBHandle xHdl, UBYTE * pubBufferOut, USHORT usBufferMax,
...
void            vMBMLogASCIIFrame( eMBPortLogLevel eLevel, xMBMInternalHandle * pxIntHdl, char *szMsg,
...
eMBMSerialASCIIInit( xMBMInternalHandle * pxIntHdl, UCHAR ucPort, ULONG ulBaudRate, eMBSerialParity eParity )
{
...
#endif
    {
        ...
        if( NULL != pxFrameHdl )
        {
            ...
            if( MB_ENOERR !=
                ( eStatus2 =
                  eMBPSerialInit( &( pxFrameHdl->xSerHdl ), ucPort, ulBaudRate, 7, eParity, ucStopBits, pxIntHdl ) ) )
            {
                eStatus = eStatus2;
            }
            ...
            if( bMBPPortLogIsEnabled( MB_LOG_DEBUG, MB_LOG_ASCII ) )
            {
                vMBPPortLog( MB_LOG_DEBUG, MB_LOG_ASCII,
                             ...
                             ( USHORT ) ucPort, ulBaudRate, ( USHORT ) eParity,
...
#endif
    {
        ...
        if( bMBPPortLogIsEnabled( MB_LOG_DEBUG, MB_LOG_ASCII ) )
        {
            vMBPPortLog( MB_LOG_DEBUG, MB_LOG_ASCII,
                         ...
                         MBP_FORMAT_USHORT ".\n", ( USHORT ) pxASCIIHdl->ubIdx, ( USHORT ) ucSlaveAddress,
                         ( USHORT ) usMBPDULength );
        }
#endif
            ...
            ( usMBPDULength <= ( MBM_SER_PDU_SIZE_MAX - ( 1 /* Slave Address */  + 1 /* LRC */  ) ) ) )
        {
            ...
            if( MB_ENOERR !=
                ( eStatus =
                  eMBPSerialTxEnable( pxASCIIHdl->xSerHdl,
                                      ( pbMBPSerialTransmitterEmptyCB ) bMBMSerialASCIITransmitterEmptyAPIV1CB ) ) )
            {
            ...
            if( MB_ENOERR !=
                ( eStatus =
                  eMBPSerialTxEnable( pxASCIIHdl->xSerHdl,
                                      ( pbMBPSerialTransmitterEmptyCB ) bMBMSerialASCIITransmitterEmptyAPIV2CB ) ) )
            {
                HDL_RESET_TX( pxASCIIHdl );
            }
#else
        ...
        if( MB_IS_VALID_HDL( pxASCIIHdl, xMBMASCIIFrameHdl ) && MB_IS_VALID_READ_ADDR( ucSlaveAddress ) )
        {
            ...
            if( NULL == pusMBPDULength )
            {
                ...
                xAnalyzerFrame.eFrameType = MB_FRAME_DAMAGED;
                     ...
                     ( ubMBMSerialASCIILRC
                       ( ( UBYTE * ) & ( pxASCIIHdl->ubASCIIFrameBuffer[0] ), pxASCIIHdl->usRcvBufferPos ) == 0 ) )
            {
                *pusMBPDULength =
...
bMBMSerialASCIITransmitterEmptyAPIV1CB( xMBHandle xHdl, UBYTE * pubValue )
{
    ...
    if( bEnableRx )
    {
        ...
        if( MB_ENOERR !=
            eMBPSerialRxEnable( pxASCIIFrameHdl->xSerHdl, ( pvMBPSerialReceiverCB ) vMBMSerialASCIIReceiverAPIV1CB ) )
        {
            eStatus = eMBPEventPost( pxIntHdl->xFrameEventHdl, MBM_EV_SEND_ERROR );
            MBP_ASSERT( MB_ENOERR == eStatus );
        }
                                        ...
                                        USHORT * pusBufferWritten )
{
    ...
    *pusBufferWritten = 0;
    ...
    if( bEnableRx )
    {
        HDL_RESET_RX( pxASCIIFrameHdl );
        if( MB_ENOERR !=
            eMBPSerialRxEnable( pxASCIIFrameHdl->xSerHdl, ( pvMBPSerialReceiverCB ) vMBMSerialASCIIReceiverAPIV2CB ) )
        {
