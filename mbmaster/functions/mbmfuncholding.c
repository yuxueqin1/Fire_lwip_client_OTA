 ...
 * All rights reserved.
 *
 * $Id: mbmfuncholding.c,v 1.24 2011/01/02 16:15:36 embedded-solutions.cwalter Exp $
 */
                               ...
                               eMBMQueryState * peState, eMBErrorCode * peStatus )
{
...
#endif
    {
        ...
        case MBM_STATE_NONE:
            if( MB_IS_VALID_WRITE_ADDR( ucSlaveAddress ) )
            {
...
#endif
    {
        ...
        case MBM_STATE_NONE:
            if( MB_IS_VALID_READ_ADDR( ucSlaveAddress ) &&
                ( ( ( ULONG ) usRegStartAddress + ( ULONG ) ubNRegs ) < ( ULONG ) 0x10000 ) &&
...
#if MBM_ENABLE_FULL_API_CHECKS == 1
            if( MB_IS_VALID_WRITE_ADDR( ucSlaveAddress ) &&
                ( ( ( ULONG ) usRegStartAddress + ( ULONG ) ubNRegs ) < 0x10000UL ) &&
