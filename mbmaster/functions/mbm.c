 ...
 * All rights reserved.
 *
 * $Id: mbm.c,v 1.39 2010/11/01 17:13:30 embedded-so.embedded-solutions.1 Exp $
 */
...
xMBMInternalHandle * pxMBMGetNewHdl( void )
{
    ...
    for( ubIdx = 0; ubIdx < MB_UTILS_NARRSIZE( xMBMInternalHdl ); ubIdx++ )
    {
        if( IDX_INVALID == xMBMInternalHdl[ubIdx].ubIdx )
        {
            ...
            if( MB_ENOERR != ( eStatus2 = eMBPTimerInit( &( pxIntHdl->xRespTimeoutHdl ),
                                                         MBM_DEFAULT_RESPONSE_TIMEOUT,
...
#endif
        else if( MB_ENOERR != ( *peStatus = pxIntHdl->pFrameSendFN( pxIntHdl, ucSlaveAddress,
...
#endif
                if( MB_ENOERR != ( *peStatus = pxIntHdl->pFrameRecvFN( pxIntHdl,
                                                                       ucSlaveAddress,
                                                                       &( pxIntHdl->usFrameMBPDULength ) ) ) )  //Mark：pFrameRecvFN
                {
...
#if MBM_ASCII_ENABLED == 1
            case MB_ASCII:
                eStatus = eMBMSerialASCIIInit( pxMBMNewIntHdl, ucPort, ulBaudRate, eParity );
                break;
#endif

#if MBM_RTU_ENABLED == 1
            case MB_RTU:
                eStatus = eMBMSerialRTUInit( pxMBMNewIntHdl, ucPort, ulBaudRate, eParity );
                break;
#endif
