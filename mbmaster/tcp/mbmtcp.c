 ...
 * All rights reserved.
 *
 * $Id: mbmtcp.c,v 1.17 2010/11/01 17:13:32 embedded-so.embedded-solutions.1 Exp $
 */
...
void            vMBMLogTCPFrame( eMBPortLogLevel eLevel, xMBMInternalHandle * pxIntHdl, char *szMsg,
...
eMBMTCPFrameInit( xMBMInternalHandle * pxIntHdl )
{
    ...
    if( NULL != pxIntHdl )
    {
        ...
        {
            if( MB_ENOERR ==
                eMBPTCPClientInit( &( pxFrameHdl->xTCPHdl ), pxIntHdl, eMBPTCPClientNewDataCB,
                                   eMBPTCPClientDisconnectedCB ) )
            {
    ...
    if( bMBMIsHdlValid( pxIntHdl ) )
    {
...
#endif
        MBP_ENTER_CRITICAL_SECTION(  );
        if( MB_IS_VALID_HDL( pxFrameHdl, xMBMTCPFrameHdl ) &&
            ...
            ( usMBPDULength <= ( MBM_TCP_PDU_SIZE_MAX - 7 /* MBAP Header */  ) ) )
        {
            if( MBP_TCPHDL_CLIENT_INVALID != pxFrameHdl->xTCPClientHdl )
            {
                ...
                eStatus2 =
                    eMBPTCPConWrite( pxFrameHdl->xTCPHdl, pxFrameHdl->xTCPClientHdl, pxFrameHdl->arubBuffer,
                                     usPayloadLength );
                ...
                case MB_ENOERR:
                    ...
                    xAnalyzerFrame.x.xTCPHeader.usMBAPTransactionId =
                        ( USHORT ) ( pxFrameHdl->arubBuffer[MBM_TCP_TID_OFF] << 8U );
                    xAnalyzerFrame.x.xTCPHeader.usMBAPTransactionId |=
                        ( USHORT ) ( pxFrameHdl->arubBuffer[MBM_TCP_TID_OFF + 1] );
                    xAnalyzerFrame.x.xTCPHeader.usMBAPProtocolId = MBM_TCP_PROTOCOL_ID;
                    xAnalyzerFrame.x.xTCPHeader.usMBAPLength =
                        ( USHORT ) ( pxFrameHdl->arubBuffer[MBM_TCP_LEN_OFF] << 8U );;
                    xAnalyzerFrame.x.xTCPHeader.usMBAPLength |=
                        ( USHORT ) ( pxFrameHdl->arubBuffer[MBM_TCP_LEN_OFF + 1] );;
...
eMBMTCPFrameReceive( xMBHandle xHdl, UCHAR ucSlaveAddress, USHORT * pusMBPDULength )
{
    ...
    if( bMBMIsHdlValid( pxIntHdl ) )
    {
        ...
        {
            if( MB_IS_VALID_HDL( pxFrameHdl, xMBMTCPFrameHdl ) && MB_IS_VALID_READ_ADDR( ucSlaveAddress ) )
            {
                ...
                else if( ( pxFrameHdl->usBufferPos >= MBM_TCP_PDU_SIZE_MIN ) &&
                         ( ucSlaveAddress == pxFrameHdl->arubBuffer[MBM_TCP_UID_OFF] ) )
                {
                    ...
                    if( ( pxFrameHdl->usCurrentTID == usMBAPTransactionIDField ) &&
                        ( MBM_TCP_PROTOCOL_ID == usMBAPProtocolID ) )
                    {
                        *pusMBPDULength = ( USHORT ) ( pxFrameHdl->usBufferPos - MBM_TCP_MB_PDU_OFF );
                        ...
                        if( bMBPPortLogIsEnabled( MB_LOG_DEBUG, MB_LOG_TCP ) )
                        {
                            ...
                            vMBMLogTCPFrame( MB_LOG_DEBUG, pxIntHdl, "Received frame: ",
                                             ( const UBYTE * )&pxFrameHdl->arubBuffer[MBM_TCP_TID_OFF],
...
eMBMTCPFrameConnect( xMBMInternalHandle * pxIntHdl, const CHAR * pcTCPClientAddress, USHORT usTCPPort )
{
    ...
    if( bMBMIsHdlValid( pxIntHdl ) && MB_IS_VALID_HDL( ( xMBMTCPFrameHandle * ) pxIntHdl->xFrameHdl, xMBMTCPFrameHdl ) )
    {
        ...
        else
        {
             ...
             */
            eStatus =
                eMBPTCPClientOpen( pxFrameHdl->xTCPHdl, &( pxFrameHdl->xTCPClientHdl ), pcTCPClientAddress, usTCPPort );
#if defined( MBM_ENABLE_DEBUG_FACILITY ) && ( MBM_ENABLE_DEBUG_FACILITY == 1 )
            if( bMBPPortLogIsEnabled( MB_LOG_DEBUG, MB_LOG_TCP ) )
            {
                vMBPPortLog( MB_LOG_DEBUG, MB_LOG_TCP,
                             "[IDX=" MBP_FORMAT_USHORT "] Connecting to client %s:" MBP_FORMAT_USHORT ": %s!\n",
                             ( USHORT ) pxFrameHdl->ubIdx, pcTCPClientAddress, usTCPPort,
    ...
    if( bMBMIsHdlValid( pxIntHdl ) && MB_IS_VALID_HDL( ( xMBMTCPFrameHandle * ) pxIntHdl->xFrameHdl, xMBMTCPFrameHdl ) )
    {
        ...
        else
        {
            ...
            if( ( pxFrameHdl->usBufferPos + usMaxBytesToRead ) < MBM_TCP_BUFFER_SIZE )
            {
                eStatus2 =
                    eMBPTCPConRead( pxFrameHdl->xTCPHdl, pxFrameHdl->xTCPClientHdl,
                                    &( pxFrameHdl->arubBuffer[pxFrameHdl->usBufferPos] ), &usBytesRead,
                                    usMaxBytesToRead );
                switch ( eStatus2 )
                {
                case MB_ENOERR:
...
#if defined( MBM_ENABLE_DEBUG_FACILITY_HEAVY ) && ( MBM_ENABLE_DEBUG_FACILITY_HEAVY == 1 )
void
vMBMLogTCPFrame( eMBPortLogLevel eLevel, xMBMInternalHandle * pxIntHdl, char *szMsg, const UBYTE * pubPayload,
                 USHORT usLength )
{
    USHORT          usIdx;
